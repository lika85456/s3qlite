...
rustfs("s3qlite sync", (it) => {
	it.effect("runs basic sync between two connections", () =>
		Effect.scoped(
			Effect.gen(function* () {
				const dbName = `test-${crypto.randomUUID().slice(0, 8)}`;
				const remoteKV = yield* makeRustfsRemoteKV("simple");
				...
				const firstUsers = yield* Effect.tryPromise(
					() =>
						first.all("SELECT id, username FROM users ORDER BY id") as Promise<
							unknown[]
						>,
				);
				const secondUsers = yield* Effect.tryPromise(
					() =>
						second.all("SELECT id, username FROM users ORDER BY id") as Promise<
							unknown[]
						>,
				);
