...
rustfs("s3qlite schema mutation sync", (it) => {
	it.effect("CREATE TABLE, ALTER TABLE ADD COLUMN, DROP TABLE across two instances", () =>
		Effect.scoped(
			Effect.gen(function* () {
				const dbName = `schema-${crypto.randomUUID().slice(0, 8)}`;
				const remoteKV = yield* makeRustfsRemoteKV("schema");
	...
	it.effect("CREATE INDEX and DROP INDEX across two instances", () =>
		Effect.scoped(
			Effect.gen(function* () {
				const dbName = `idx-${crypto.randomUUID().slice(0, 8)}`;
				const remoteKV = yield* makeRustfsRemoteKV("schema-index");
				const localA = yield* makeMemoryKV();
				const localB = yield* makeMemoryKV();
